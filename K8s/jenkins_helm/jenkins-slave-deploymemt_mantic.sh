...
#read VOL_NAME
kubectl create configmap "$DPLY_NAME"-config  --from-file=config-map-slave_mantic -o yaml --dry-run=client | kubectl apply -n devops-tools -f -

helm install $DPLY_NAME jenkinsagent_mantic  --set  volumes.volName="$DPLY_NAME"-vol --set  volumes.claimName="$DPLY_NAME"-volclaim --set service.name="$DPLY_NAME"-service --set deployment.name="$DPLY_NAME"-deployment --set  service.app="$DPLY_NAME"-app  --set deployment.app="$DPLY_NAME"-app --set host.name="$DPLY_NAME"-slave --set configmap.name="$DPLY_NAME"-config
