...
helm install $DPLY_NAME jenkinsagent  --set volumes.claimName="$DPLY_NAME"-volclaim --set service.name="$DPLY_NAME"-service --set deployment.name="$DPLY_NAME"-deployment --set  service.app="$DPLY_NAME"-app  --set deployment.app="$DPLY_NAME"-app --set host.name="$DPLY_NAME"-slave --set configmap.name="$DPLY_NAME"-config
